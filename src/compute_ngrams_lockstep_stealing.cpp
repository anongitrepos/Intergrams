...
int main(int argc, char** argv)
{
  ...
  if (argc != 3)
  {
    std::cerr << "Usage: " << argv[0] << " directory/ <stealer_threads>" << std::endl;
    exit(1);
  }
  ...
  const int stealerThreads = atoi(argv[2]);

  c.tic();
  {
    // This will start getting chunks immediately.
    std::filesystem::path d(argv[1]);
    LockstepStealingReaderThread reader(d, 3);
    usleep(100); // this may not be enough??
    {
      // Fire off the thread to process the n-grams.
      {
        std::vector<NgramLockstepThread<LockstepStealingReaderThread>*> ngrams;
        for (size_t t = 0; t < 64; ++t)
          ngrams.emplace_back(new
              NgramLockstepThread<LockstepStealingReaderThread>(reader, t,
              gram3Counts, globalWaitingForChunk, globalFlushTime));

        std::vector<WorkStealerThread*> stealers;
        for (size_t t = 0; t < stealerThreads; ++t)
          stealers.emplace_back(new WorkStealerThread(reader, ngrams));
